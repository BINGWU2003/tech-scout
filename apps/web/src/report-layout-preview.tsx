...
for (const c of companies) {
  ...
  client.setQueryData(['research', 'preview', 'patents', c.id, 1], {
    items: [
      {
        id: 'CN10001',
        title: '一种储能系统及其能量管理方法',
        year: 2025,
        dateKind: 'publication',
        abstract: '通过动态调整充放电策略提升储能系统运行效率。',
        cpcs: ['H02J'],
        domains: [],
        source: { url: null, sha256: null },
      },
...
for (const id of ['CN10001', 'CN10002'])
  client.setQueryData(['research', 'preview', 'patent', id], {
    items: [
      {
        id,
        title: '一种储能系统及其能量管理方法',
...
function Preview() {
  return (
    <QueryClientProvider client={client}>
      <div className='flex h-dvh flex-col gap-4 bg-background p-4 text-foreground sm:p-6'>
        <nav className='grid shrink-0 grid-cols-4 gap-2'>
          {['研究计划', '专利检索', '企业发现', '研究报告'].map((x, i) => (
            <div
              key={x}
              className={`rounded-lg px-3 py-2.5 text-center text-xs sm:text-sm ${i === 3 ? 'bg-primary text-primary-foreground' : 'bg-muted/40 text-muted-foreground'}`}
            >
              {i + 1}. {x}
            </div>
          ))}
        ...
        <header className='shrink-0'>
          <h1 className='text-2xl font-semibold'>研究报告</h1>
        ...
        <ReportWorkspace
          ...
          controls={
            <p className='text-sm'>
              当前研究{' '}
              <span className='ml-2 rounded bg-muted px-2 py-1'>已完成</span>
            </p>
          }
          records={<ResearchTimeline events={[]} active={false} />}
        />
      </div>
    </QueryClientProvider>
  )
