...
数据底座、Data Gate 和第一批产品只读查询已完成，可以开始 Agent 工具开发：
...
| App             | 用户账户和数据库 Session；未来承载项目等数据 |                  2 张业务表 | 账户功能已启用 |
...
| `app`     |    3 | Prisma migration、账户与 Session      | 是，NestJS 读写  |
...
`app` 已由 NestJS 侧的 Prisma migration 建立账户基础表：
...
2026-09-03 在本地 PostgreSQL 的当前 `2026-09-v6` 上完成 Kysely repository warm smoke。每类查询执行 30 次，列表目标 p95 不超过 500 ms、详情目标 p95 不超过 800 ms。
...
全部达标，阶段 1 不增加推测性索引。该结果是本地 repository 基线，不代替部署环境的端到端监控。

## 14. 后续推荐

当前不需要继续清洗已有基础数据。推荐顺序：

1. 在 Python Intelligence 中实现只读 Catalog 工具和内部 API。
2. 建立最小研究工作流、checkpoint、人工确认和 SSE 事件链。
3. 需要保存用户项目和研究结果时，再设计 `app` migration。
4. 只有具体功能需要时，才补摘要、权利要求、专利族、新闻或财务数据。
