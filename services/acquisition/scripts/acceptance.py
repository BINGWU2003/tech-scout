...
from tech_scout_acquisition.__main__ import loop_factory
...
async def run(marker):
    config = settings()
    config.acquisition_database_url = type(config.acquisition_database_url)(
        make_conninfo(
...
if __name__ == "__main__":
    ...
    if MARKER.exists():
        ...
        with psycopg.connect(
            settings().acquisition_database_url.get_secret_value(), autocommit=True
        ) as conn:
            conn.execute(
