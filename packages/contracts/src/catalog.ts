...
export const catalogPatentSummarySchema = z.object({
  ...
  patentDate: z.iso.date().nullable(),
  publicationYear: z.number().int().nullable().optional(),
  grantYear: z.number().int().nullable(),
...
export const catalogSourceReferenceSchema = z
  .object({
    ...
    sourceRowNumber: z.number().int().min(0).nullable(),
...
export const catalogCompanyDetailSchema = z.object({
  ...
  provider: z.string().trim().min(1),
  entityStatus: z.string().nullable(),
  businessInfo: z.record(z.string(), z.string()).optional(),
